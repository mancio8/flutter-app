...
class RifornimentiNotifier extends AsyncNotifier<List<Rifornimento>> {
  ...
  Future<void> addRifornimento(Rifornimento rifornimento) async {
    ...
    try {
      ...
      if (veicoloSelezionato != null) {
        final rifornimenti = await repository.getRifornimentiPerVeicolo(veicoloSelezionato);
...
final rifornimentiStatsProvider = Provider<RifornimentiStats>((ref) {
  ...
  return rifornimentiAsync.when(
    data: (rifornimenti) {
      final totaleSpeso = rifornimenti.fold<double>(0, (sum, r) => sum + r.costo);
      final totaleLitri = rifornimenti.fold<double>(0, (sum, r) => sum + r.litri);
      final prezzoMedio = totaleLitri > 0 ? (totaleSpeso / totaleLitri).toDouble() : 0.0;
    ...
    loading: () => const RifornimentiStats(totaleSpeso: 0, totaleLitri: 0, prezzoMedio: 0),
    error: (_, __) => const RifornimentiStats(totaleSpeso: 0, totaleLitri: 0, prezzoMedio: 0),
  );
});
